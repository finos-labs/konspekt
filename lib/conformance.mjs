...
export function loadInstance(instanceDir, opts = {}) {
  ...
  // An append-only `| entity | commit | file |` table binding each committed code
  // change to the entity it was about, one row per (entity, commit, file) in
  // commit order. `commit` is an OPAQUE revision token — validated for shape only,
  ...
  if (changeLog) {
    ...
    if (existsSync(chPath)) {
      ...
      for (const line of readFileSync(chPath, "utf8").split("\n")) {
        ...
        row++;
        const [entity, commit, file] = cells;
        const target = byId.get(entity);
        if (!target) {
